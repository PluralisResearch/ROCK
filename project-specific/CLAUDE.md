...
- **ROCK Gateway**: Running (port 8080) — proxy + SQLite trace logging + trace query API
...
### Gateway Server
- `ROCK/rock/sdk/model/server/config.py` — `ModelServiceConfig` (host, port, proxy_base_url, retryable_status_codes, request_timeout, trace_db_enabled)
- `ROCK/rock/sdk/model/server/utils.py` — `@record_traj` decorator — writes enhanced traces to JSONL
...
- `ROCK/rock/sdk/model/server/trace_store.py` — SQLite trace storage
- `ROCK/rock/sdk/model/server/api/traces.py` — `GET /v1/traces`, `/v1/traces/{id}`, `/v1/traces/stats`
